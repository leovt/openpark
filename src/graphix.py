...
class GlProgram:
    ...
    def vertex_attrib_pointer(self, buffer, name, size, type=gl.GL_FLOAT, normalized=False, stride=0):
        loc = gl.glGetAttribLocation(self.handle, ctypes.create_string_buffer(name))
        gl.glEnableVertexAttribArray(loc)
        gl.glBindBuffer(gl.GL_ARRAY_BUFFER, buffer)
        gl.glVertexAttribPointer(loc, size, type, normalized, stride, ctypes.c_void_p(0))
